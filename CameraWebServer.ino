...
// L_EN, R_EN, L_PWM, R_PWM
BTS7960 motorControllerL(35, 41, 36, 37);
BTS7960 motorControllerR(2, 42, 3, 46);

void setup() {
  // Serial.begin(9600);
  // Serial.println("Motors");
  motorControllerL.Enable();
  motorControllerR.Enable();
}

void loop() {

  for (int speed = 0; speed < 255; speed += 10) {
    motorControllerL.TurnLeft(speed);
    motorControllerR.TurnLeft(speed);
    delay(100);
  }
  
  for (int speed = 255; speed > 10; speed -= 10) {
    motorControllerL.TurnLeft(speed);
    motorControllerR.TurnLeft(speed);
    delay(100);
  }
  ...
  for (int speed = 0; speed < 255; speed += 10) {
    motorControllerL.TurnRight(speed);
    motorControllerR.TurnRight(speed);
    delay(100);
  }
